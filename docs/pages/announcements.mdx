...
we will apply the default SSL options to `tls.connect`, and rejectUnauthorized` will be enabled by default and it might result in a failed connection when using self-signed certificates. This makes pg a bit more secure "out of the box" while still enabling you to opt in to the old behavior.
